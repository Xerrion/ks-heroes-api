...
def normalize_troop_stats(input_path: str, output_path: str | None = None):
    ...
    troopType -> troopLevel -> TG_level -> stats"""
    ...
    # Iterate over each troop type section dynamically
    for troop_type, entries in data.items():
        troop_dict = defaultdict(dict)

        for entry in entries:
            level = int(entry["Troop Level"])
            tg = int(entry["FC level"])  # Rename FC -> TG

            # Clean unwanted metadata keys
            stats = {
                k: v
        ...
        # Sort all numeric keys properly
...
if __name__ == "__main__":
    ...
    normalize_troop_stats(input_file, output_file)
    print("Done. All troop types normalized.")
